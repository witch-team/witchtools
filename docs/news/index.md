...
- [`convert_gdx()`](http://witchtools.witchmodel.org/reference/convert_gdx.md)
  could not be called without explicitly passing `region_mappings` and
  `time_mappings`. Both defaults referred to the parameters themselves
  rather than to the package data, so any other call failed with
  “promise already under evaluation”. The documented example could not
  run.
- [`witch_query()`](http://witchtools.witchmodel.org/reference/witch_query.md)
  failed for any `add_year` other than `"t30"`: a malformed
  `if (!is.null)` test (missing its argument) raised “invalid argument
  type”.
- [`witch_translate_data()`](http://witchtools.witchmodel.org/reference/witch_translate_data.md)
  could not generate its GAMS include files. `witch_write_gams()`
  ignored the time mapping it was given and read an undefined `time_id`
  instead, erroring after writing part of the output.
- [`premise_region_mapping()`](http://witchtools.witchmodel.org/reference/premise_region_mapping.md)
  ignored its `n` argument and always grouped by the `witch17` column,
  so any other mapping failed with “group length is 0”.
- `guess_scenario()` returned the whole path as the scenario name when
  given Windows-style paths on Linux or macOS, where `\` is not a path
  separator.
- [`witch_scen_name()`](http://witchtools.witchmodel.org/reference/witch_scen_name.md)
  and `guess_scenario()` treated `".gdx"` as a regular expression,
  stripping the first four-character run ending in `gdx` anywhere in the
  name instead of the extension.
- `witch_results_files(normalize = TRUE)` resolved file names against
  the working directory rather than `search_path`, so it warned and
  returned un-normalized names whenever the two differed.
- [`archive_store()`](http://witchtools.witchmodel.org/reference/archive_store.md)
  could not overwrite an existing archive on macOS or Windows: the
  pre-copy delete targeted the wrong path, so
  [`fs::file_copy()`](https://fs.r-lib.org/reference/copy.html) failed
  with `EEXIST`.

### Other changes

- Loading the package no longer overrides `witchtools.*` options that
  the user has already set.
- `remotes` moved from `Imports` to `Suggests`; it is only needed to
  install `gdxtools` from GitHub and was already installed on demand.
- Removed the unused internal `paleiee()` palette helper, which
  referenced a `witch_regions` dataset that does not exist and could not
  run.
...
- Update `witch_translate_data` to automatically process DuckDB files
  (`data_*.duckdb`)
- Improve `require_package` to automatically install missing packages in
  non-interactive sessions
- Add helpful error messages when no writable R library directory is
  available

## witchtools 0.4.12

- Use rlang::check_installed to check for and to install required
  packages
...
- In GAMS, add 2 coalition variables to deal with a high number of
  coalitions.
- Add witch20, ed57 aggregation

## witchtools 0.4.7

- Add ‘tagg’ in metaparam to control the function to aggregate over
  time.
...
- Add guess_scenario function to guess the scenario name from a list of
  gdx
...
- Remove DVC support as default for `witch_data`. Now, just return the
  filename.
...
- Update `convert_region` to allow downscaling with `sumby`. It was not
  possible before.
